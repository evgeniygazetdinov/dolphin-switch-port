...
//   Call rc_api_process_X_response to parse the server's response
//   Clean up with rc_api_destroy_request on the generic struct
    ...
    const std::function<int(RcResponse*, const char*)>& process_response)
{
  ...
  int result = init_request(&api_request, &rc_request);
  if (result != RC_OK)
    return ResponseType::INVALID_REQUEST;

  Common::HttpRequest http_request;
  auto response = http_request.Post(api_request.url, api_request.post_data);
  ...
  result = process_response(rc_response, std::string(response.begin(), response.end()).c_str());
  if (result != RC_OK)
    return ResponseType::UNKNOWN_FAILURE;

  return ResponseType::SUCCESS;
}

// Explicit template instantiations
template AchievementManager::ResponseType
AchievementManager::Request<rc_api_fetch_game_data_request_t, rc_api_fetch_game_data_response_t>(
    rc_api_fetch_game_data_request_t, rc_api_fetch_game_data_response_t*,
    const std::function<int(rc_api_request_t*, const rc_api_fetch_game_data_request_t*)>&,
    const std::function<int(rc_api_fetch_game_data_response_t*, const char*)>&);

template AchievementManager::ResponseType
AchievementManager::Request<rc_api_fetch_user_unlocks_request_t, rc_api_fetch_user_unlocks_response_t>(
    rc_api_fetch_user_unlocks_request_t, rc_api_fetch_user_unlocks_response_t*,
    const std::function<int(rc_api_request_t*, const rc_api_fetch_user_unlocks_request_t*)>&,
    const std::function<int(rc_api_fetch_user_unlocks_response_t*, const char*)>&);
